...
function setupTouchControls() {
    ...
    // Tap detection for dropping the piece
    body.addEventListener('touchstart', function (event) {
        const now = new Date().getTime();
        const timeSinceLastTouch = now - (body.lastTouchTime || 0);
        ...
        body.lastTouchTime = now;
    }, false);
}

// Ensure to call this function during initialization
setupTouchControls();

// Add the setupTouchControls function to the Init function
function Init () { 
    Page.Initialize();
    GM.Initialize();
    setupTouchControls();  // Add this line
}
